...
class FOURIER(QAOA):
    ...
    def get_angles_FOURIER_constant(G, p, q, backend, R=0, initial_u = None, initial_v = None, n_samples = 1024): 
        '''
        ...
        for i in tqdm(range(1,p+1)):
            u, v = FOURIER.find_local_amplitudes(G, initial_u, initial_v, i, backend, n_samples = n_samples)            
            initial_u, initial_v = u.copy(), v.copy()
        
        return FOURIER.translate_fourier_to_angles(u, v, p)
    
    def get_angles_FOURIER_infinity(G, p, backend, R=0, initial_u = None, initial_v = None, n_samples = 1024):
